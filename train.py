...
if __name__ == '__main__':
    ...
    # 学习率默认值
    best_learning_rate = 1.7
    best_decay_rate = 1/1000
    plot_x = []
    plot_y = []
    best_acc = 0
    # 参数查找
    for i in range(1, 21, 2):
        learning_rate = i / 10
        accs = []
        for k in [10, 100, 1000, 5000, 10000, 20000]:
            network, final_acc, train_loss1, test_loss1, test_accuracy1, train_iterations = \
                training(x_train1, y_train1, x_valid1, y_valid1, lr=learning_rate, dr=1 / k)
        ...
        plot_y.append(max(accs))

    print(best_acc)
    print("best learning rate: " + str(best_learning_rate))
    print("best decay rate: " + str(best_decay_rate))

    host = host_subplot(131)
    ...
    # 绘制曲线
    p1, = host.plot(plot_x, plot_y, "b-", label="validation accuracy")
    p2, = host.plot(plot_x, plot_y, ".")  # 曲线点
    ...
    for i in [400, 225, 100, 64, 25]:
        network, final_acc, train_loss1, test_loss1, test_accuracy1, train_iterations = \
            training(x_train1, y_train1, x_valid1, y_valid1, lr=best_learning_rate, dr=best_decay_rate,hs=i)
        if (final_acc > best_acc):
            best_network = network
            best_hidden_size = i
            best_acc = final_acc
            ...
            test_loss = test_loss1
            test_accuracy = test_accuracy1
        plot_x.append((np.sqrt(i)))
        plot_y.append(final_acc)

    host = host_subplot(132)
    plt.subplots_adjust(right=0.8)  # ajust the right boundary of the plot window
    print(plot_x)
    print(plot_y)
    # 设置类标
    host.set_xlabel("Hidden Size")
    host.set_ylabel("accuracy")
    ...
    print(best_acc)
    print("best hidden size: " + str(best_hidden_size))
    plot_x = []
    plot_y = []
    # lambda默认值
    best_lambda = 0.01
    for i in range(1, 10):
        lam = i * (10 ** (-2))
        network, final_acc, train_loss1, test_loss1, test_accuracy1, train_iterations = \
            training(x_train1, y_train1, x_valid1, y_valid1, lr=best_learning_rate, dr=best_decay_rate,
                     hs=best_hidden_size, lambd=lam)
        if (final_acc > best_acc):
            best_network = network
            best_lambda = lam
            best_acc = final_acc
        ...
        plot_y.append(final_acc)
    # 绘制曲线图
    host = host_subplot(133)
    plt.subplots_adjust(right=0.8)  # ajust the right boundary of the plot window

    # 设置类标
    host.set_xlabel("Regularization Strength")
    host.set_ylabel("accuracy")

    # 绘制曲线
    p3, = host.plot(plot_x, plot_y, "b-", label="validation accuracy")
    p4, = host.plot(plot_x, plot_y, ".")  # 曲线点
    ...
    plt.show()
    print(best_acc)
    print("best regularization strength: " + str(best_lambda))
    ...
    # 设置范围
    host.set_xlim([-10, 5000])
    plt.draw()
    plt.show()

# See PyCharm help at https://www.jetbrains.com/help/pycharm/
    torch.save(best_network,'model.pth')
    model = torch.load('model.pth')
    w1 = model.params['w1']
    w2 = model.params['w2']
    pca = PCA(n_components=3)
    mat1 = pca.fit_transform(w1) # 每个样本降为3维　
    dim = int(np.sqrt(w1.shape[1]))
    for i in range(w1.shape[1]):
        x1 = normalization(w1[:,i])
        img = x1.reshape(28,28)
        plt.subplot(dim,dim,i+1)
        plt.imshow(img,cmap="gray")
    plt.show()
    for i in range(dim):
        x2 = normalization(w2[:,i])
        img = x2.reshape(dim,dim)
        plt.subplot(1,dim,i+1)
        plt.imshow(img,cmap="gray")
    plt.show()
    mat1_re = normalization(mat1.reshape(28,28,3))
    mat2 = pca.fit_transform(w2)
    mat2_re = normalization(mat2.reshape(dim,dim,3))
    imsave("img1.jpg",mat1_re)
    imsave("img2.jpg",mat2_re)
